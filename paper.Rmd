...
The data came from two sources; satellite data and in situ data from public
sources. The satellite data was downloaded utilizing Google Earth Engine, which
...
chlorophyll values, and the data also includes secchi depth (how deep you can
see into the water). 
...
Using a merge threshold, the in situ data was shifted forward and backward $0,
...
+ 1$, and $k + 2$. In other words, the data from any single in situ observation
  would be used for the surrounding four days. The merge threshold procedure was
  done up to ten days of threshold. 
...
closer in space or time will covary.
...
help account for the temporal structure in the data. Unfortunately, there was no
explicit spatial variable that could be included in the data to account for
spatial dependence.

The Random Forest and Bosted Tree implementation from `scikit-learn` were used.
...
were repoted on the test data set.
...
Finally, we generate chlorophyll estimates for each of the available satellite images,
...
feature, the inclusion of a feature describing lake elevation will be important 
