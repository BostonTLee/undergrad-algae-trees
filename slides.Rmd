...
- The algae bloom data is spatiotemporal: We have measurements that are potentially correlated over geographic space, that are taken over time
- This means we cannot use any regression-based methods. Assumptions about uncorrelated errors are violated.
...
- We can interpret residuals as the **negative gradient** of the total loss defined before.
- Like AdaBoost, we update the prediction function, but now, we update $F(\cdot)$ based on the negative gradient of our total loss.
- This algorithm can handle different loss functions, with different properties, making it very flexible.
- Here, we use the default implementation in `scikit-learn`, which has a least-squares loss function:
